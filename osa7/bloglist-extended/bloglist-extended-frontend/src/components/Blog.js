...
const Blog = forwardRef((props, ref) => {
  ...
  */
  ...
  const handleRemoveBlog = (event) => {
    event.preventDefault()
    console.log('handleRemoveBlog, blog selected for removal: ', props.blog.title)
  ...
  if (viewallbloginfo === false) {
    ...
    return (
      <li className="singleblog">
        <h3>
          <span style={{ margin: '8%' }}>Title</span>{' '}
          <span style={{ margin: '8%' }}>Author</span>{' '}
          <span style={{ margin: '8%' }}>URL</span>{' '}
          <span style={{ margin: '8%' }}>Likes</span>
        </h3>
        <span className="singleblogfield" id="list-blog-title">
          {props.blog.title}
        </span>
        <span style={{ paddingTop: '30%' }}>
          <button type="button" onClick={handleAddBlogLike}>
            <b>like this blog</b>
          </button>
        </span>
      </li>
    ...
    if (props.currentUser.username === props.blog.user.username) {
      ...
      return (
        <li className="singleblog">
          ...
          <span style={{ paddingLeft: '30%' }}>
            <button type="submit" onClick={handleAddBlogLike}>
              <b>Like this blog</b>
            </button>
          </span>
          <span style={{ paddingLeft: '30%' }}>
            <button type="submit" onClick={handleRemoveBlog}>
              <b>Remove this blog</b>
            </button>
          </span>
        </li>
      )
      ...
      return (
        <li className="singleblog">
          ...
          <span className="singleblogfield">{props.blog.title}</span>
          <h3>
            <span>Author</span>
          </h3>
          <span className="singleblogfield">{props.blog.author}</span>
          <h3>
            <span>URL</span>
          </h3>
          <span className="singleblogfield">{props.blog.url}</span>
          <h3>
            <span>Likes</span>
          </h3>
          <span className="singleblogfield">{props.blog.likes}</span>
          <span style={{ paddingLeft: '30%' }}>
            <button type="submit" onClick={handleAddBlogLike}>
              <b>Like this blog</b>
            </button>
