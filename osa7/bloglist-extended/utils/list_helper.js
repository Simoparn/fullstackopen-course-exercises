...
const mostBlogs = (blogs) => {
  const unique_authors_array = lodash._.uniqBy(blogs, function (blog) {
    return blog.author
  })
  ...
  blogs_by_author_array = blogs_by_author_array.fill(
    0,
    0,
    unique_authors_array.length
  )
  ...
  let most_blogs_author =
    unique_authors_array[
      blogs_by_author_array.indexOf(Math.max(...blogs_by_author_array))
    ]
  console.log('Author with most blogs: ', {
    author: most_blogs_author.author,
    blogs: Math.max(...blogs_by_author_array),
  })
  return {
    author: most_blogs_author.author,
    blogs: Math.max(...blogs_by_author_array),
...
const mostLikes = (blogs) => {
  ...
  likes_by_author_array = likes_by_author_array.fill(
    0,
    0,
    unique_authors_array.length
  )
  ...
  let most_likes_author =
    unique_authors_array[
      likes_by_author_array.indexOf(Math.max(...likes_by_author_array))
    ]

  return {
    author: most_likes_author.author,
    blogs: Math.max(...likes_by_author_array),
  }
}
