describe('Blog app tests 5.17-5.22', function() {
  ...
  describe('when logged in', function() {
    ...
    it('5.18 Login fails with wrong credentials', function() {
      ...
      cy.login({ username: 'testusername1', password: 'testpassword167' })
    ...
    it('5.19 a new blog can be created', function() {
      ...
      cy.get('#list-blog-title').contains('a blog created by cypress')
      .parent().should('contain', 'Author')
      .and('contain', 'title')
      .and('contain', 'URL')
      .and('contain', 'Likes')
    ...
    it('5.20 a new blog can be liked', function() {
      ...
      cy.get('#list-blog-title').contains('a blog created by cypress')
      .parent().find('button').contains('Like this blog').click()
    ...
    it('5.21 a new blog can be deleted', function() {
      ...
      cy.get('#list-blog-title').contains('a blog created by cypress')
      .parent().should('contain', 'Like this blog')
      .and('contain', 'Remove this blog')
      
      cy.get('#list-blog-title').contains('a blog created by cypress')
      .parent().find('button').contains('Remove this blog').click()
    ...
    describe('if several blogs are added', function () {
      beforeEach(function () {
        ...
        cy.createBlog({ title: 'first blog with cypress', author: 'author', url: 'testurl', likes:4 })      
        cy.createBlog({ title: 'second blog with cypress', author: 'author', url: 'testurl', likes:8 })      
        cy.createBlog({ title: 'third blog with cypress', author: 'author', url: 'testurl', likes:12 })
      })

      it('one of these can be viewed fully', function () {
        cy.contains('second blog with cypress').parent().find('button').as('theButton')
        cy.get('@theButton').click()
