...
test('5.16 <BlogForm /> calls the received prop callback function with the right data when a blog is created', async () => {
  ...
  console.log("5.16 test: mockCreateBlog calls content:", mockCreateBlog.mock.calls[0][0].title)
  expect(mockCreateBlog.mock.calls[0][0].title).toBe('testing the form for adding a new blog title...')
  expect(mockCreateBlog.mock.calls[0][0].author).toBe('testing the form for adding a new blog author...')
  expect(mockCreateBlog.mock.calls[0][0].url).toBe('testing the form for adding a new blog url...')
